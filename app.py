...
def extract_questions(question_pdf: bytes) -> Tuple[List[Question], Dict[int, bytes]]:
    ...
    # Render every page for preview/cropping.
    for pi, page in enumerate(doc, start=1):
        pix = page.get_pixmap(matrix=fitz.Matrix(1.6, 1.6), alpha=False)
        page_images[pi] = pix.tobytes("png")

    # Detect groups from full text.
    full_text = "\n".join(page.get_text("text") for page in doc)
    groups = []
    for m in re.finditer(r"請閱讀[^。\n]*?並回答\s*(\d{1,2})\s*[～~\-至]\s*(\d{1,2})\s*題", full_text):
        a, b = int(m.group(1)), int(m.group(2))
        groups.append((a, b))

    # Extract questions page by page using text lines with coordinates.
    for pi, page in enumerate(doc, start=1):
        ...
        for block in d.get("blocks", []):
            ...
            for line in block.get("lines", []):
                ...
                if t:
                    ...
                    lines.append((bbox[1], bbox[0], t, bbox))
        lines.sort(key=lambda x: (x[0], x[1]))

        starts = []
        for idx, (_, _, t, bbox) in enumerate(lines):
            m = re.match(r"^(\d{1,2})\.\s*(.*)", t)
            if m and 1 <= int(m.group(1)) <= 99:
                starts.append((idx, int(m.group(1)), bbox))

        for si, (idx, qno, bbox) in enumerate(starts):
            next_idx = starts[si+1][0] if si+1 < len(starts) else len(lines)
            chunk = [x[2] for x in lines[idx:next_idx]]
            # Stop at obvious footer or next section labels
            cleaned = []
            for t in chunk:
            ...
            # Separate options from stem.
            opt_matches = list(re.finditer(r"(?:^|\n)\s*[\(（]([ABCD])[\)）]\s*", raw))
            options = {}
            if opt_matches:
                stem = raw[:opt_matches[0].start()].strip()
                for oi, om in enumerate(opt_matches):
                    key = om.group(1)
                    start = om.end()
                    end = opt_matches[oi+1].start() if oi+1 < len(opt_matches) else len(raw)
                    options[key] = raw[start:end].strip()
            else:
                stem = raw.strip()

            # Crop from this question's top to next question top.
            y0 = max(0, bbox[1] - 8)
            if si+1 < len(starts):
                y1 = max(y0 + 50, starts[si+1][2][1] - 8)
            else:
                y1 = min(page.rect.height, y0 + 330)
            crop_rect = fitz.Rect(0, y0, page.rect.width, min(page.rect.height, y1))
            cpix = page.get_pixmap(matrix=fitz.Matrix(1.6, 1.6), clip=crop_rect, alpha=False)
            crop = cpix.tobytes("png")

            # Visual mode if the crop includes image blocks intersecting question area.
            visual = False
            for block in d.get("blocks", []):
                if block.get("type") == 1:
                    br = fitz.Rect(block.get("bbox"))
                    if br.intersects(crop_rect):
                        visual = True
                        break
    ...
    # De-duplicate, keep earliest occurrence.
    uniq = {}
    for q in sorted(questions, key=lambda x: (x.source_no, x.page_no)):
        if q.source_no not in uniq:
            uniq[q.source_no] = q
    return list(uniq.values()), page_images
...
with tab1:
    ...
    if st.button("建立／更新題庫", type="primary", disabled=not (qfile and afile and rfile)):
        ...
        st.success(f"完成：辨識 {len(questions)} 題；答案 {len(answers)} 題；通過率 {len(rates)} 題。")
